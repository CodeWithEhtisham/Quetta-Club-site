...
def customers(request):
    if request.method == 'POST':
        ...
        sales =Sales.objects.select_related('customer_id').values('customer_id').annotate(total_amount=Sum('net_amount'))
        ids=[i['customer_id'] for i in sales]
        amount=[i['total_amount'] for i in sales]
        cs=Customers.objects.all().annotate(total=Value(0,output_field=CharField()))
        for idx,i in enumerate(list(cs)):
            if i.id in ids:
                cs[idx].total=amount[ids.index(i.id)]
...
def customer_details(request):
    return render(request, "Customers/customer_details.html", {
        ...
        'all_bills': Sales.objects.exclude(net_amount=0).filter(customer_id__id=request.GET.get("id")).count(),
        'total_amount': Sales.objects.exclude(net_amount=0).filter(customer_id__id=request.GET.get("id")).aggregate(Sum('amount'))['amount__sum'],
        'total_discount': Sales.objects.exclude(net_amount=0).filter(customer_id__id=request.GET.get("id")).select_related('bill_no').aggregate(Sum('discount'))['discount__sum'],
