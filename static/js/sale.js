  ...
  Object.keys(data).forEach(key => {
    ...
    if (elem['amount'] > 0) {
      row = '<tr>' +
        ...
        '<td>' +
        '<div class="list">' +
        '<a href="/update_view_sale/?id=' + elem['id'] + '" style="background-color: #1e659c; color: white;">Edit</a>' +
        ...
        '<a href="/delete_sale/'+elem['id']+'" style="background-color: #ae180d; color: white;">Delete</a>'+
        '</div>' +
        '</td>' +
        '</tr>'
      all_rows += row;
...
function paidModalSubmit() {
  ...
  total_amount=$('#pay_bill_modal_balance').val();
...
function compModalSubmit() {
  ...
  total_amount=$('#comp_bill_modal_balance').val();
  paid_amount=$('#comp_amount').val();
  remaining_amount=total_amount-paid_amount;
  let data;
  if (remaining_amount<0)  {
