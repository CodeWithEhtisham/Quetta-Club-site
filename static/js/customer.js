  ...
  Object.keys(data).forEach((key) => {
    ...
    if (elem["amount"] > 0) {
      row =
        ...
        "</td>" +
        "<td>" +
        elem["customer_id"]["customer_rank"] +
        "</td>" +
        "<td>" +
        elem["PoS_no"] +
        "</td>" +
        "<td>" +
        elem["customer_id"]["customer_name"] +
        "</td>" +
        ...
        "<td>" +
        '<div class="list">' +
        '<a href="/Sales/update_sales/?id=' +
        elem["id"] +
        '" style="background-color: #1e659c; ">Edit</a>' +
        '<button class="modal" id="paid_modal" onclick="paidMOdalOpen(' +
        elem["id"] +
        "," +
        elem["net_amount"] +
        ')" style="background-color: #1e659c; ">Paid</button>' +
        '<button id="comp_modal" class="modal" onclick="compModalOpen(' +
        elem["id"] +
        "," +
        elem["net_amount"] +
        ')">Complemantery</button>' +
        '<button id="cancel_modal" class="modal" onclick="cancelModalOpen(' +
        elem["id"] +
        "," +
        elem["net_amount"] +
...
document.getElementById("comp-today-date").value =
  compToday.getFullYear() +
  "-" +
  ("0" + (compToday.getMonth() + 1)).slice(-2) +
  "-" +
  ("0" + compToday.getDate()).slice(-2);

let cancelToday = new Date();
document.getElementById("cancel-today-date").value =
  cancelToday.getFullYear() +
  "-" +
  ("0" + (cancelToday.getMonth() + 1)).slice(-2) +
  "-" +
...
function paid_all_bills() {
  ...
  if (check == true) {
    ...
    let datetime = window.prompt("enter Date", "day-month-year");
    console.log(rv, datetime);
    if (rv == null || rv == "" || datetime == null || datetime == "") {
      alert("Please enter RV number and Date");
      return false;
    } else {
